...
function getDirectorySummary(changedFiles) {
  ...
  const componentMapping = {
    'publish.py': 'Core RTMP/Video Publishing Script',
    'raspberry_pi': 'Raspberry Pi Installation/Configuration',
    'nvidia_jetson': 'NVIDIA Jetson Support',
    'orangepi': 'Orange Pi Support',
    ...
    'raspininja.service': 'System Service Configuration'
  };
  ...
  for (const { file } of changedFiles) {
    ...
    if (file.includes('install') || file.includes('setup')) knownComponents.add('Installation/Setup');
    if (file.includes('service')) knownComponents.add('System Service');
    if (file.includes('firmware')) knownComponents.add('Firmware Updates');
    if (file.includes('cargo') || file.includes('rust')) knownComponents.add('Rust Configuration');
  }
...
* **Purpose:** Turns Raspberry Pi and similar single-board computers into wireless HDMI video capture devices for live streaming and remote production.
* **Core Features:** RTMP/WebRTC video publishing, low-latency streaming, multi-platform support (Raspberry Pi, NVIDIA Jetson, Orange Pi), hardware encoding support, system service management, remote control capabilities.
